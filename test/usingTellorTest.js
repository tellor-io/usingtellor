...
describe("UsingTellor", function() {
  ...
  it("getCurrentValue()", async function() {
    await playground.connect(addr1).submitValue(h.uintTob32(1),150,0,'0x')
    blocky1 = await h.getBlock()
    currentVal1 = await bench.getCurrentValue(h.uintTob32(1))
    expect(currentVal1[0])
    expect(currentVal1[1]).to.equal(h.bytes(150))
    expect(currentVal1[2]).to.equal(blocky1.timestamp)
    await playground.connect(addr1).submitValue(h.uintTob32(1),160,1,'0x')
    blocky2 = await h.getBlock()
    currentVal2 = await bench.getCurrentValue(h.uintTob32(1))
    expect(currentVal2[0])
    expect(currentVal2[1]).to.equal(h.bytes(160))
    expect(currentVal2[2]).to.equal(blocky2.timestamp)
    await playground.submitValue(h.hash("abracadabra"), h.bytes("houdini"), 0, h.bytes("abracadabra"))
    blocky3 = await h.getBlock()
    currentVal3 = await bench.getCurrentValue(h.hash("abracadabra"))
